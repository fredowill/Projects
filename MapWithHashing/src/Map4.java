...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final Pair<K, V> remove(K key) {
        ...
        this.size--;
        int index = mod(key.hashCode(), this.hashTable.length);
        Pair<K, V> temp = this.hashTable[index].remove(key);
        return temp;
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        Map<K, V> temp = this.hashTable[0];

        int i = 0;
        while (this.hashTable[i].size() > 0) {
            temp.add(this.hashTable[i].removeAny());
            i++;
        }

        return temp.removeAny();
    }
    ...
    public final V value(K key) {
        ...
        V temp;
        int index = mod(key.hashCode(), this.hashTable.length);
        temp = this.hashTable[index].value(key);
        return temp;
