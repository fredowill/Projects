...
public class Map2<K, V> extends MapSecondary<K, V> {
    ...
    private static <K, V> void moveToFront(Queue<Pair<K, V>> q, K key) {
        ...
        Pair<K, V> tempP = new SimplePair<K, V>(key, null);
        Pair<K, V> tempP2 = new SimplePair<K, V>(key, null);

        for (int i = 0; i < length; i++) {
            Pair<K, V> p = new SimplePair<K, V>(key, null);
            p = q2.dequeue();
            if (p.key().equals(key)) {
                tempP = p;
            } else {
    ...
    public final V value(K key) {
        ...
        Pair<K, V> y = new SimplePair<K, V>(key, null);
        moveToFront(this.pairsQueue, key);

        y = this.pairsQueue.dequeue();
