...
public final class Queue1LSort4<T> extends Queue1L<T> {
    ...
    private static <T> void partition(Queue<T> q, T partitioner, Queue<T> front,
        ...
        /*
         * loop till q has elements that is compared to and checked that if
         * temp/ current value is smaller than or equal to partitioner it is put
         * in front otherwise back
         */
        while (q.length() > 0) {
            T temp = q.dequeue();
    ...
    public void sort(Comparator<T> order) {
        ...
        if (this.length() > 1) {
             ...
             */
            //im not sure how we decide the partition value, also i just did this
            // idk any other way to select an entry from a queue
            T temp = this.dequeue();
             ...
             */
            partition(this, temp, front, back, order);
