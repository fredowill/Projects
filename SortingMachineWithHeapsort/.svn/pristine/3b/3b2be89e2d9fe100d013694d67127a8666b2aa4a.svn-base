...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        /*
         * to check if a heap with size 2 is in the correct order
         */
        if (left == last) {
            ...
            if (order.compare(array[left], array[right]) > 0) {
                /*
                 * here left is greater than the right root so we exchange
                 * entries with the right root
                 */
                if (order.compare(array[top], array[right]) > 0) {
                    exchangeEntries(array, top, right);
                    siftDown(array, right, last, order);
                }
            } else if (order.compare(array[left], array[right]) <= 0) {

                /*
                 * not checking if left and right are the same, but if left is
                 * smaller exchange with the left entry and top
                 */
                if (order.compare(array[top], array[left]) > 0) {
                    exchangeEntries(array, top, left);
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // the array /heap is still not a heap because the elements might be out of order
    ...
    public final void changeToExtractionMode() {
        ...
        //now we actually build and initialize the array and heapSize to length
